...
test('Pi loads package and skill, tools run, and the plan gate protects chapter prose', async t => {
  ...
  const tool = (name: string) => {
    const entry = extension.tools.get(name); assert.ok(entry, `tool ${name} registered`); return entry.definition;
  };
  ...
  // 方案未批准之前，正文必须写不进去
  const beforePlan = JSON.parse(await call('novel_read', { path: chapter.path }));
  await assert.rejects(
    call('novel_write', { path: chapter.path, expectedRevision: beforePlan.revision, body: '# 雨夜\n\n抢写。' }),
    /方案尚未被作者批准/,
  );
  ...
  // 批准是作者命令，不是工具 —— 这里直接调用内核模拟 /novel approve
  const project = new Project(book);
  const proposed = await project.read(planPath);
  await project.transition(planPath, 'confirm', proposed.revision);
  ...
  await assert.rejects(call('novel_write', { path: chapter.path, expectedRevision: read.revision, body: 'stale' }), /版本冲突/);
...
test('guard allows only known safe tools; output is explicitly bounded', () => {
  ...
  assert.ok(allowedTool('novel_propose'));
