...
派生资料（状态、事件、关系变化、审稿）**必须**有 `sources`，否则拒绝创建。
...
**不适用于章节两件套**（走 `novel_new_chapter`），也不适用于 `export`（工具生成）。

拒绝的情况：未知种类、章节两件套、`export`、派生资料缺 `sources`、`refs` 指向不存在的编号。

### `novel_new_chapter`

一次生成 `方案.md` / `正文.md` 两份文档，分配两个编号，并自动排到最后一章之后。
...
派生种类（状态、事件、关系变化、审稿）不能收编 —— 那些必须有来源版本。
...
**因此正文的版本会变** —— 指向它的派生记录会自动重绑到新版本（这一点是刻意的：
路径无关不等于内容无关）。
